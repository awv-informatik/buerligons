...
export const Gizmo: React.FC<{ drawingId: DrawingID; productId: ObjectID; matrix: THREE.Matrix4 }> = ({
  ...
  const onDragStart = React.useCallback(
    ({ component }: { component: 'Arrow' | 'Slider' | 'Rotator' }) => {
      ...
      ccApi(drawingId).v0.assemblyBuilder.startMovingUnderConstraints(curProdId, draggedInstances, pivotPos, mucType)
    },
  ...
  const transformInstances = React.useCallback(
    async (mdL_: THREE.Matrix4) => {
      ...
      promise = ccApi(drawingId).v0.assemblyBuilder.moveUnderConstraints(curProdId, rot, offset).catch(console.warn)
      await promise
  ...
  const onDragEnd = React.useCallback(() => {
    ...
    mdL.current = null
    const curProdId = getDrawing(drawingId).structure.currentProduct || -1
    ccApi(drawingId).v0.assemblyBuilder.finishMovingUnderConstraints(curProdId)
  }, [drawingId])
