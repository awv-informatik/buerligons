    ...
    (type: 'Part' | 'Assembly') => {
      const run = async () => {
        try {
          ...
          const newDrawingId = await ClassCAD.utils.connect(type)
          if (newDrawingId) {
            switch (type) {
              case 'Assembly':
...
const undoCommand = (drawingId?: DrawingID, states?: States): Command => {
      ...
      ? filteredStack
          ...
          .map(state => ({
            ...
            command: () => {
              ...
              stateToLoad && ClassCAD.utils.undo(drawingId, stateToLoad)
            },
          }))
      : []
  ...
  return {
    ...
    command: () => drawingId && ClassCAD.utils.undo(drawingId),
  }
}
...
const redoCommand = (drawingId?: DrawingID, states?: States): Command => {
    ...
    drawingId && states?.current && states.stack && states.captionMap
      ? states.stack
          ...
          .map(state => ({
            label: getCaption(state, states),
            stateId: state,
            command: () => ClassCAD.utils.redo(drawingId, state),
          }))
      : []
  ...
  return {
    ...
    command: () => drawingId && ClassCAD.utils.redo(drawingId),
  }
}
